...
class TestToken(TestBase):
    ...
    def test_verify_token_not_exist(self):
        response = self.client.get(
            url_for('verify', tok="626dec4f-c87a-4e41-8f50-cedce221f5c7"))
...
class TestGenToken(TestBase):
    def test_gen_new_token(self):
        with requests_mock.Mocker() as bingoreq:
            bingoreq.get('http://bingo:5000/uid/' + "2",
                         text='[{"id":2,"user_first_name":"Jane","user_last_name":"Grandy","user_login_name":"janeg"}]')
    ...
    def test_gen_update_user(self):
        with requests_mock.Mocker() as bingoreq:
            bingoreq.get('http://bingo:5000/uid/' + "1",
                         text='[{"id":1,"user_first_name":"Jane","user_last_name":"Grandy","user_login_name":"janeg"}]')
