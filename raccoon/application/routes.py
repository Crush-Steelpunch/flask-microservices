...
def getuser(user_id):
    datain = request.get_json()  # expecting { "token":"uuid","searchkey":"searchterm"}
...
def addinfo(user_id):
    datain = request.get_json() # expecting { "token":"uuid", "user_info_key":"keyname","user_info_value":"{json data}"}
    verified_token = token_verify(datain["token"])
    if not verified_token:
        adddata = UsersInfo(user_id=user_id,user_info_key=datain["user_info_key"],user_info_value=datain["user_info_value"])
